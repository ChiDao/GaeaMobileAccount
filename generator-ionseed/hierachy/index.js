...
module.exports = yeoman.Base.extend({
  ...
  prompting: {
    configFile: function () {
      ...
      this.prompt(
      {
        ...
        name    : 'configFile',
        message : '配置文件路径',
        default : this.sourceRoot() + '/hierachy.json' // Default to current folder name
      }, function (answers) {
        var vim = execFile('vim',[answers.configFile]);
        vim.on('close', function(){
          var hierachyConfig = fs.readJSONSync(answers.configFile);
          console.log(hierachyConfig);
          done();
        });
      }.bind(this));
    },
  ...
  install: {
  	modifileFile: function () {
	    ...
	    //   "$1\n\n" +
	    //   "      .state('tab." + this.objectNameDasherize + "', {\n" +
	    //   "      url: '/" + this.objectsStateUrl + "/:" + this.objectName + "Id' ,\n" +
	    //   "      views: {\n" +
	    //   "        'tab-" + this.objectsNameDasherize + "': {\n" +
	    ...
	    //   /(stateProviderPlaceHolder)/,
	    //   "$1\n\n" +
	    //   "      .state('tab." + this.objectsNameDasherize + "', {\n" +
